...
7. **Host Integration**: Defines the lower level integration of the game engine with content packs, possibly including render pipelines, network packets, and other engine-level integration points.
8. **Diagnostics**: Defines the diagnostic and debugging capabilities that can be provided by the game engine to content packs, including logging, error reporting, profiling, and other diagnostic tools.
...
Refers to a piece of game content metadata registered with a host through an OBGX interface, such as a record, block, or entity.
...
When this terminology occurs without context, it refers to a value used to identify a [game object](#game-object).
