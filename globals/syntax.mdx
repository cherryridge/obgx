...
The notation of a type is defined on this page. The abstract meaning of that type is defined by the [Type System](type), and behavior that occurs when an operation is performed is defined by [Execution](execution).
...
The following words are reserved and MUST NOT be used as user-defined identifiers:

- `type` and `enum` are declaration keywords.
- `i8`, `i16`, `i32`, `i64`, `u8`, `u16`, `u32`, `u64`, `f32`, `f64`, `bool`, `string`, and `hndl` are fundamental type names.
- `void` is a special type name that can only appear as the return type of a function type or function declaration.
- `any` is the type wildcard used in type patterns.
...
An object declaration consists of an identifier, `:`, a type expression, and a required semicolon:
...
A compound type declaration starts with `type`, followed by the type identifier and a brace-delimited sequence of field declarations. Each field declaration and the closing brace must be followed by a semicolon.
...
An enum declaration starts with `enum`, followed by the type identifier and a non-empty, brace-delimited, comma-separated list of member identifiers. A trailing comma is not permitted. The closing brace must be followed by a semicolon.
...
A function declaration consists of a function identifier, a parenthesized parameter list, `:`, a return type, and a required semicolon. Parameters use the same form and comma rules as function types.
