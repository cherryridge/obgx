...
Each implementation must provide at least one way to construct a `refid` from a game object identifier, and at least one way to construct a `refid` from an identifier and a namespace identifier.
...
An implementation must provide at least one way to determine an enum value's active member.

For every member that has a payload, an implementation must also provide at least one way to obtain that payload when the value is that member. OBGX does not prescribe the API or language construct used for either operation.
...
An implementation must provide at least one way to determine an array's length. OBGX does not prescribe the API or language construct used to obtain an array's length.
...
An implementation must provide at least one way to determine a vector's length. OBGX does not prescribe the API or language construct used to obtain a vector's length.
