...
`f32` and `f64` are floating-point types. Their exact finite value domains and whether they include special values such as `NaN` and infinities remain unspecified pending the floating-point decision described in [Floating-Point Arithmetic](execution#floating-point-arithmetic).
...
**When observed through an OBGX interface**, a `string` should behave as a sequence of UTF-8 code points.
...
A host must provide at least one way to determine whether an enum value's active member is any of the member of the enum type. For every member that has a payload, the host must also provide at least one way to obtain that payload when the value is that member. OBGX does not prescribe the API or language construct used for either operation.
...
An array is an ordered, indexed collection of values of one element type. Every array has a fixed, positive length. Its valid indices are the unsigned integer values from `0` through `length - 1`.
