...
The OBGX type system defines the abstract types and values used by OBGX interfaces. It is independent of any host language or storage format. **Unless explicitly stated otherwise, the underlying representation of a value is implementation-defined**.

The standardized textual forms for these types are defined by [Syntax](syntax). Behavior produced by operations on values is defined by [Execution](execution).
...
The host must provide at least one way to construct a `refid` from a game object identifier, and at least one way to construct a `refid` from an identifier and a namespace identifier.

When constructing a `refid` with a game object identifier (the first case in the previous paragraph), the host must use the namespace associated with the caller pack to construct the `refid`.

OBGX does not prescribe the API or language construct used to construct `refid`.
...
The host must provide at least one way to determine an enum value's active member.

For every member that has a payload, the host must also provide at least one way to obtain that payload when the value is that member. OBGX does not prescribe the API or language construct used for either operation.
...
The host must provide at least one way to determine an array's length. OBGX does not prescribe the API or language construct used to obtain an array's length.
...
The host must provide at least one way to determine a vector's length. OBGX does not prescribe the API or language construct used to obtain a vector's length.

### Function Value

A function value is an object that can be invoked with a specific set of argument types and has a specific return type.
